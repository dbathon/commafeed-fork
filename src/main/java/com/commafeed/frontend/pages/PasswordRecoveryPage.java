...
public class PasswordRecoveryPage extends BasePage {
  ...
  public PasswordRecoveryPage() {
    ...
    final Form<String> form = new Form<String>("form", email) {
      ...
      protected void onSubmit() {
        super.onSubmit();
        final User user = userDAO.findByEmail(getModelObject());
        if (user == null) {
  ...
  private String callbackUrl(User user, String publicUrl) throws Exception {
    return new URIBuilder(publicUrl)
        .addParameter(PasswordRecoveryCallbackPage.PARAM_EMAIL, user.getEmail())
        .addParameter(PasswordRecoveryCallbackPage.PARAM_TOKEN, user.getRecoverPasswordToken())
