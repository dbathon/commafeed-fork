...
public class Subscription implements Serializable {
  ...
  public static Subscription build(FeedSubscription subscription, String publicUrl, long unreadCount) {
    ...
    final FeedCategory category = subscription.getCategory();
    ...
    sub.setNextRefresh((feed.getDisabledUntil() != null && feed.getDisabledUntil().before(now))
        ? null : feed.getDisabledUntil());
  ...
  @ApiProperty(value = "next time the feed refresh is planned, null if refresh is already queued",
      required = true)
