...
public class ModelFactory {
  ...
  public static <T> String invokedProperty(T proxiedValue) {
    final Argument<T> a = ArgumentsFactory.actualArgument(proxiedValue);
    return a.getInkvokedPropertyName();
  }

  public static <T> PropertyModel<T> model(Object value, T proxiedValue) {
    final String invokedPN = invokedProperty(proxiedValue);
    final PropertyModel<T> m = new PropertyModel<T>(value, invokedPN);
    return m;
  }

  @SuppressWarnings("unchecked")
  public static <T> T proxy(T t) {
    final Object object = Lambda.on(t.getClass());
    return (T) object;
  }
