...
public class FaviconFetcher {
  ...
  protected static List<String> ICON_MIMETYPES = Arrays.asList("image/x-icon",
      "image/vnd.microsoft.icon", "image/ico", "image/icon", "text/ico", "application/ico",
      "image/x-ms-bmp", "image/x-bmp", "image/gif", "image/png", "image/jpeg");
  private static List<String> ICON_MIMETYPE_BLACKLIST = Arrays.asList("application/xml",
      "text/html");
  ...
  private byte[] getIconInPage(String url) {
    ...
    final Elements icons = doc.select("link[rel~=(?i)^(shortcut|icon|shortcut icon)$]");
    ...
    final String href = icons.get(0).attr("abs:href");
